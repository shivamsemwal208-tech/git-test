...
function WeatherPanel() {
  ...
  return (
    <>
        ...
        <div className="inline-flex items-center gap-1 rounded-lg border border-white/10 bg-white/[.03] p-1">
          <button
            ...
            className={`rounded-md px-3 py-1.5 text-[11px] font-bold tracking-wide transition-colors ${
              showingScenario ? 'bg-command/20 text-command-soft' : 'text-slate-400 hover:text-slate-200'
            }`}
          >
            SIMULATION · SCENARIO
          </button>
      ...
      <div className="grid min-w-0 gap-5 lg:grid-cols-[minmax(0,1.2fr)_minmax(0,.8fr)]">
        <section className="panel p-5">
          <div className="flex items-center gap-2">
            <CloudRain className="text-command-soft" />
        ...
        <section className="panel p-5">
          <Gauge className="text-command-soft" />
      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        {displayedMetrics.map(([label, value]) => (
          <div key={label} className="tile border-l border-l-command/0 transition-colors duration-150 hover:border-l-command/60">
