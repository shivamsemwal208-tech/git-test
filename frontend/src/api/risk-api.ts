...
/** Raw backend historical-event record from the `historical` block. */
export interface RiskHistoricalEvent {
  fid: number
  ...
  flood_impact_index: number | null
}

/** Raw backend `historical` intelligence block from POST /risk/assess. */
export interface RiskHistoricalContext {
  ...
  coverage: {
    ...
    usable_event_count: number | null
  } | null
  search_radius_m: number
  distance_basis: 'ANCHOR_VERTEX'
  nearest: RiskHistoricalEvent | null
  polygon_contains_location: boolean | null
  events_nearby: RiskHistoricalEvent[]
...
export interface RiskAssessment {
  ...
  /** Phase 3C: recorded-flood intelligence. Present for live results only. */
  historical?: RiskHistoricalContext | null
}
...
export interface RiskCommand {
  ...
  explanation: string | null
  disclaimer: string | null
  /** PRESENT for live risk results, null for demo/simulation/unreachable. */
...
export function mapRiskAssessment(
  ...
  return {
    ...
    historical: mapHistoricalContext(assessment.historical),
  }
}

/** Maps a backend snake_case historical-event record to the camelCase view. */
export function mapHistoricalEvent(record: RiskHistoricalEvent): HistoricalEvent {
  return {
    fid: record.fid,
    reportNumber: record.report_number,
    distanceM: record.distance_m,
    distanceBasis: record.distance_basis,
    latitude: record.latitude ?? null,
    longitude: record.longitude ?? null,
    beginDate: record.begin_date ?? null,
    endDate: record.end_date ?? null,
    country: record.country,
    cause: record.cause,
    severity: record.severity ?? null,
    floodImpactIndex: record.flood_impact_index ?? null,
  }
}

/** Maps the backend `historical` block; null when absent (demo results). */
export function mapHistoricalContext(
  block: RiskHistoricalContext | null | undefined,
): HistoricalContext | null {
  if (!block) return null
  return {
    status: block.status,
    statusReason: block.status_reason ?? null,
    source: block.source ?? null,
    coverage: block.coverage
      ? {
          startDate: block.coverage.start_date ?? null,
          endDate: block.coverage.end_date ?? null,
          eventCount: block.coverage.event_count ?? null,
          usableEventCount: block.coverage.usable_event_count ?? null,
        }
      : null,
    searchRadiusM: block.search_radius_m,
    distanceBasis: block.distance_basis,
    nearest: block.nearest ? mapHistoricalEvent(block.nearest) : null,
    polygonContainsLocation: block.polygon_contains_location ?? null,
    eventsNearby: (block.events_nearby ?? []).map(mapHistoricalEvent),
    disclaimer: block.disclaimer ?? null,
  }
}
...
/**
 * Formats a river-distance value (metres) for display: `<1000 m` stays in
 * metres ("867 m"), `>=1000 m` is shown in kilometres with two decimals
 * ("1.28 km"), and a missing value renders as "Unavailable" — never a
 * fabricated "0 m". Mirrors the backend's honesty rule that an unmapped river
 * answer is not silently zero.
 */
export function formatRiverDistance(meters: number | null): string {
