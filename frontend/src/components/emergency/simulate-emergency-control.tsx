...
export function SimulateEmergencyControl() {
  ...
  return (
    <div className="rounded-lg border border-sev-high/35 bg-sev-high/[0.07] p-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <p className="flex items-center gap-2 text-sm font-black tracking-[.13em] text-sev-high-soft">
          <FlaskConical size={16} />
          {emergencySimulation ? 'EMERGENCY SIMULATION ACTIVE' : 'SIMULATE EMERGENCY'}
        </p>
        <DataStatusBadge label="DEMO / SIMULATION" tone="amber" />
      </div>
      <p className="mt-3 text-xs leading-5 text-sev-high-soft/80">
        {emergencySimulation
      ...
      <button
        ...
        className="mt-4 inline-flex items-center gap-2 rounded-lg border border-sev-high/45 bg-sev-high/15 px-4 py-2 text-sm font-bold text-sev-high-soft transition hover:bg-sev-high/25"
      >
        {emergencySimulation ? (
          <>
      ...
      {emergencySimulation && (
        <p className="mt-3 text-[10px] leading-4 text-sev-high-soft/60">
