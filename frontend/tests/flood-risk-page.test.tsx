...
describe('FloodRiskPage — LIVE ML prediction (arbitrary coordinates)', () => {
  ...
  it('shows the real river distance in metres below 1000 m', () => {
    renderFloodRisk(
      {
        ...livePredictionPayload,
        terrain: { ...livePredictionPayload.terrain, river_distance: 867 },
      },
      arbitraryLocation,
    )
    expect(screen.getByText('867 m')).toBeInTheDocument()
  })

  it('shows the real river distance in kilometres at 1000 m and above', () => {
    renderFloodRisk(
      {
        ...livePredictionPayload,
        terrain: { ...livePredictionPayload.terrain, river_distance: 1279 },
      },
      arbitraryLocation,
    )
    expect(screen.getByText('1.28 km')).toBeInTheDocument()
  })

  it('never fabricates a river distance when the value is missing', () => {
    renderFloodRisk(
      {
        ...livePredictionPayload,
        terrain: { ...livePredictionPayload.terrain, river_distance: null },
      },
      arbitraryLocation,
    )
    const riverCard = within(screen.getByText('River distance').closest('div') as HTMLElement)
    expect(riverCard.getByText('Unavailable')).toBeInTheDocument()
    expect(riverCard.queryByText(/0 m/)).not.toBeInTheDocument()
  })
})
