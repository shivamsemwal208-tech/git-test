...
# Canonical schema order for deterministic missing-feature diagnostics. The
# artifact may declare its features in any order; the response always lists
# them in FEATURE_NAMES order so callers see a stable, predictable list.
...
def _canonical_missing_order(missing: list[str]) -> list[str]:
    """Sort missing-feature names into canonical schema order, de-duplicated.

    Unknown names sort after the known ones. This keeps the ``DATA_INCOMPLETE``
    ``missing_features`` list deterministic no matter which order the artifact
    declares its required features in, so callers/storage see a stable list.
    """
    return sorted(
        set(missing), key=lambda name: _FEATURE_ORDER.get(name, len(FEATURE_NAMES))
    )
...
def predict(vector: dict, *, path: Path | str | None = None) -> dict:
    ...
    for name in loaded.feature_names:
        ...
        if value is None or not math.isfinite(float(value)):
            missing.append(name)
    # Deterministic diagnostics: canonical schema order, de-duplicated.
    missing = _canonical_missing_order(missing)
    if missing:
        return _unavailable(
            "DATA_INCOMPLETE",
