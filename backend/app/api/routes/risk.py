...
from backend.app.data_sources import history_provider
...
def _historical_terrain_string(historical: dict) -> str:
    """Concise, non-contradictory legacy ``terrain.historical`` string.

    The detailed historical intelligence lives in the ``historical`` block;
    this legacy terrain string stays truthful and consistent with it.
    """
    if historical.get("status") != "AVAILABLE":
        return "Unavailable"
    nearest = historical.get("nearest")
    if nearest is None:
        radius_km = int(historical.get("search_radius_m", 0) / 1000)
        return f"No recorded event within {radius_km} km"
    year = (nearest.get("begin_date") or "")[:4]
    km = nearest["distance_m"] / 1000
    return f"Recorded {year} event ~{km:.1f} km ({nearest['report_number']})"
...
def _ml_risk_assessment(
    ...
    *,
    ...
    # Reuse the weather payload fetched above for the feature engine too, so the
    # live provider is queried once per request (the display weather and the ML
    # features are the same Open-Meteo call).
    components = features.live_components(latitude, longitude, current=current)
    ...
    # Historical flood intelligence (Phase 3C): informational only, sourced
    # from the local DFO event index. Never an ML feature.
    historical = history_provider.historical_context(latitude, longitude)
    terrain["historical"] = _historical_terrain_string(historical)
