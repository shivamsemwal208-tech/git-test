...
def clear_weather_cache():
    ...
    river_provider._segments_cache.clear()
    river_provider._index = True
    history_provider._cache.clear()
    history_provider._index = True
    usgs_provider._cache.clear()
    safe_place_provider._cache.clear()
    yield
    weather_service._cache.clear()
    elevation_provider._cache.clear()
    terrain_provider._cache.clear()
...
@pytest.fixture
def river_error():
    return river_provider.ProviderError("River index read failed: mocked failure")
...
def mock_river(monkeypatch, request):
    ...
    The canned mock returns ``None`` by default — mirroring an honest "no
    mapped river near / river feature unavailable" state for the optional
    feature (the real HydroRIVERS-backed provider is exercised for real by
    ``test_live_sources`` and ``test_river_index``). Tests can reconfigure via
...
@pytest.fixture(autouse=True)
def mock_history(monkeypatch, request):
    """Deterministic fake for the historical-intelligence provider (autouse).

    Defaults to an honest ``UNAVAILABLE`` state — mirroring a fresh checkout
    where the DFO event index has not been built yet — so the existing live
    ML tests never depend on ``data/processed/dfo_event_index``. Tests can
    reconfigure via ``mock_history.configure(block=...)``.

    ``test_history_provider`` and ``test_risk_history`` exercise the real
    provider against tiny synthetic indexes instead, and ``test_live_sources``
    exercises real provider transports, so the mock is skipped for those
    modules.
    """
    if request.node.path.name in (
        "test_history_provider.py",
    ...
    state = {
        "block": {
            ...
            "events_nearby": [],
            "disclaimer": (
                "The local DFO event index is missing or unreadable, so "
                "historical flood intelligence is unavailable."
            ),
        },
        "error": None,
    }
    ...
    def fake_fetch(latitude, longitude, radius_m=50000.0):
        calls.append(
    ...
    monkeypatch.setattr(history_provider, "historical_context", fake_fetch)
    yield SimpleNamespace(
        calls=calls,
        state=state,
        configure=lambda block=None, error=None: _set_history_mock(state, block, error),
    )


def _set_history_mock(state, block, error):
