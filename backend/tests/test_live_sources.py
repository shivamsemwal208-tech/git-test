...
providers) with monkeypatched transports; the river provider is exercised for
real against a tiny on-disk synthetic index (see ``test_river_index.py`` for
the exhaustive index behaviour). Route-level behavior is covered in
``test_arbitrary.py`` and ``test_routes.py``.
"""
import httpx
import pytest
...
# ─── River provider (real HydroRIVERS-backed GeoIndex) ────────────────────

@pytest.fixture
def tiny_river_index(tmp_path, monkeypatch):
    """Build a small on-disk grid index of one segment far from everything."""
    from backend.app.data_sources import river_index

    segments = [(78.0322, 30.3165, 78.0422, 30.3265)]
    river_index.write_index(
        segments,
        tmp_path,
        cell_size_deg=0.1,
        source="synthetic test segment",
        build_time_iso="2026-01-01T00:00:00Z",
    )
    monkeypatch.setenv("FLASHGUARD_RIVER_INDEX_DIR", str(tmp_path))
...
def test_fetch_river_returns_distance_when_river_near(tiny_river_index):
    value = river_provider.fetch_nearest_river_distance(
        30.3165, 78.0322, radius_m=15000.0
    )
    assert isinstance(value, int)
    assert value >= 0


def test_fetch_river_none_never_claims_about_water(tiny_river_index):
    # Far from the single synthetic segment: honest None, never a guess.
    value = river_provider.fetch_nearest_river_distance(
        24.5, 88.5, radius_m=15000.0
    )
    assert value is None


def test_fetch_river_is_short_ttl_cached(tiny_river_index):
    first = river_provider.fetch_nearest_river_distance(30.32, 78.03)
    second = river_provider.fetch_nearest_river_distance(30.32, 78.03)
    assert first == second
    assert second is not None


def test_fetch_river_none_when_index_missing(tmp_path, monkeypatch):
    monkeypatch.setenv("FLASHGUARD_RIVER_INDEX_DIR", str(tmp_path / "missing"))
    river_provider._index = True
