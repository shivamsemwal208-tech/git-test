...
* ``slope_degrees`` / ``aspect_degrees``: derived deterministically from a 3x3
  ...
  flat.
* ``river_distance_m``: optional proximity feature with NO source connected in
  this release (see ``river_provider``). It is always ``None`` — honest "no
  river source", never a claim about water presence. It is excluded from the
  11-feature model contract and never blocks a prediction when unavailable.
...
FEATURE_SCHEMA: tuple[FeatureSpec, ...] = (
    ...
    _spec("river_distance_m", "m", "terrain",
          "Distance to the nearest mapped waterway. NO source is connected in "
          "this release, so the value is always None (never a guess about "
          "water presence). Optional: excluded from the 11-feature model "
          "contract and never blocks a prediction.", optional=True),
)
...
def live_components(latitude: float, longitude: float) -> dict:
    ...
    pulled from the river provider, which returns ``None`` in this release
    (optional feature; never blocks a prediction). Every failure degrades to
