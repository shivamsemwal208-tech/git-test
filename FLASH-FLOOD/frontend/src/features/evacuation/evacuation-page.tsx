    ...
    <>
      ...
      <div className="mt-4 grid gap-5 lg:grid-cols-[.75fr_1.25fr]">
        <section className="border border-white/8 bg-white/[.03] p-6">
          <p className="text-[11px] font-bold tracking-[.15em] text-cyan-200">SELECT DESTINATION</p>
            ...
            {safePlaces.map((place) => {
              ...
              return (
                <button
                  ...
                  className={`flex w-full items-start gap-3 border px-3 py-2.5 text-left ${active ? 'border-cyan-300/60 bg-cyan-300/10' : 'border-white/8 bg-white/[.02] hover:border-white/20'}`}
                >
                  <span className={`mt-1.5 h-2 w-2 shrink-0 rounded-full ${active ? 'bg-cyan-300' : 'bg-slate-600'}`} />
                  <span>
          ...
          <p className="mt-4 border-t border-white/8 pt-4 text-[11px] font-bold tracking-[.15em] text-cyan-200">
          ...
          <p className="mt-5 text-sm font-semibold text-amber-100">{route?.status}</p>
        ...
        <section>
          ...
          <div className="mt-3 flex gap-2 text-xs text-slate-500">
            <Navigation size={15} className="text-cyan-200" />
