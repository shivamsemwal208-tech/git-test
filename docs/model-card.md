...
## Recorded history is context, not a feature

The `historical` block in live risk responses (DFO v0.9.0 event index) is
informational only. It is **never** part of the model feature contract
(`MODEL_FEATURE_NAMES` / `FEATURE_NAMES` are unchanged), never changes a
prediction, and — because labels for training are DFO-derived — presenting history
alongside a prediction is descriptive context, not an accuracy claim. No
flood-frequency statistics, no "no record implies no flood" conclusions, and no
in-sample/out-of-sample confusion: live responses clearly separate the ML
prediction (`prediction_status`) from the recorded-history block (`historical`).